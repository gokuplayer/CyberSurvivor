...
public class KatanaSpawnerScript : MonoBehaviour, ISpawner
{
    ...
    void Update()
    {
        ...
        {
            GameObject katana = Instantiate(s_Weapon, transform.position + offsetVector, transform.rotation);
            Rigidbody2D katanaRb = katana.GetComponent<Rigidbody2D>();

            Vector2 katanaDirection = direction.normalized;
            ...
            Vector2 adjustedVelocity = (katanaDirection * projectileVelocity) + new Vector2(playerXVelocity, playerYVelocity);
            ...
            katanaRb.velocity = adjustedVelocity;

            if (s_Level == 7)
            {
                GameObject katana2 = Instantiate(s_Weapon, transform.position + offsetVector, Quaternion.Inverse(transform.rotation));
                Rigidbody2D katanaRb2 = katana2.GetComponent<Rigidbody2D>();
                katanaRb2.velocity = Quaternion.Euler(0, 0, 45) * adjustedVelocity;
            }
