...
public class DroneSpawnerScript : MonoBehaviour, ISpawner
{
    ...
    void Update()
    {
        ...
        {
            GameObject drone = Instantiate(s_Weapon, transform.position + offsetVector, transform.rotation);
            Rigidbody2D droneRb = drone.GetComponent<Rigidbody2D>();

            // Calculate the normalized direction towards the mouse cursor
            Vector2 droneDirection = direction.normalized;
            ...
            Vector2 adjustedVelocity = (droneDirection * projectileVelocity) + new Vector2(playerXVelocity, playerYVelocity);
            ...
            droneRb.velocity = adjustedVelocity;
