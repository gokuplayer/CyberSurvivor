...
public class MinigunSpawner : MonoBehaviour, ISpawner
{
    ...
    void Update()
    {
        ...
        {
            GameObject minigun = Instantiate(s_Weapon, transform.position + offsetVector, transform.rotation);
            Rigidbody2D minigunRb = minigun.GetComponent<Rigidbody2D>();

            Vector2 minigunDirection = direction.normalized;
            ...
            Vector2 adjustedVelocity = (minigunDirection * projectileVelocity) + new Vector2(playerXVelocity, playerYVelocity);
            ...
            minigunRb.velocity = adjustedVelocity;
